...
class Resource:
    ...
    def initialise_resources(self):
        ...
        for _dir in (
            ...
            if self.dir_is_present(_dir):
                print(f"{_dir} directory present")
            else:
                os.makedirs(_dir)
                print(f"Created : {_dir} directory")
        if self.file_is_present(self.student_details):
            print(f"{self.student_details} is present")
        else:
            with open(
            ...
            print(f"Created : {self.student_details} directory")
        if self.file_is_present(self.trained_data_label):
            print(f"{self.trained_data_label} is present")
        ...
        if self.file_is_present(self.today_attendance_file_name):
            print(f"{self.today_attendance_file_name} is present")
        else:
            with open(
            ...
            print(f"Created : {self.today_attendance_file_name} attendance file")
    ...
    def has_training_data(self):
        if self.file_is_present(self.trained_data_label):
            ...
            print(f"{self.trained_data_label} is not present")
            return False
    ...
    def image_classifier(self) -> str:
        return os.path.join(self.resource_dir, "haarcascade_frontalface_default.xml")
...
class CameraThread(QThread):
    ...
    def run(self):
        if not self.valid_request():
            print("Invalid data to proceed further. Returning!")
            return
...
class StudentRegisterFeedThread(CameraThread):
    def tear_down(self):
        print("tearing down setup for student registration")
        if self.status:
            self.status = False
            self.cam_feed.release()
            cv2.destroyAllWindows()
            print("resetting field values in registration page")
...
class StudentAttendanceFeedThread(CameraThread):
    ...
    def tear_down(self):
        print("tearing down setup for student Attendance")
        if self.status:
            self.status = False
            self.cam_feed.release()
            cv2.destroyAllWindows()
...
class MainWindow(QMainWindow):
    ...
    def start_model_training(self):
        ...
        print("All Images trained on model")
        widgets.home_title_label.setText("Smart Attendance System")
    ...
    def prepare_attendance_report(self):
        ...
        print(f"Records in app: {app_table_record_count}")
        ...
        for index, row_data in today_report_df.iterrows():
            print(row_data['Id'], row_data['Name'], row_data["Date"], row_data["Time"])
